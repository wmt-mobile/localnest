...
const TOOLS_DIR = join(__dirname, '..', 'mcp', 'tools');

/**
 * Count registerJsonTool() calls across all tool definition files.
 * @returns {number}
 */
function countToolRegistrations() {
  try {
    const files = readdirSync(TOOLS_DIR).filter(f => f.endsWith('.js') && f !== 'index.js');
    let total = 0;
...
export const TOOL_COUNT = countToolRegistrations();
