 ...
 * Mirrors the logic in src/app/create-services.js but only boots memory.
 */
...
async function handleAdd(args, opts) {
  ...
  const result = await svc.storeEntry({
    content,
    kind: flags.type || 'knowledge',
    importance: flags.importance !== undefined ? flags.importance : 50,
    nest: flags.nest || '',
    branch: flags.branch || '',
    title: flags.title || '',
  });
...
async function handleList(args, opts) {
  ...
  const useJson = opts.json || flags.json;
...
async function handleShow(args, opts) {
...
async function handleDelete(args, opts) {
  const { flags, positionals } = parseFlags(args, {
