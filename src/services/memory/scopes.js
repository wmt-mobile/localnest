 ...
 */
export async function writeDiaryEntry(adapter, { agentId, content, topic }) {
 ...
 */
export async function readDiaryEntries(adapter, { agentId, topic, limit = 20, offset = 0 }) {
  if (!agentId || typeof agentId !== 'string' || !agentId.trim()) {
  ...
  const countRow = await adapter.get(
