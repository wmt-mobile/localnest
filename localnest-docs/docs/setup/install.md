...
- The current runtime uses `@huggingface/transformers` for local embedding and reranking execution. New setup defaults use the `huggingface` provider, and older `xenova` configs remain accepted as a compatibility alias.
...
- `0.0.4` removes the earlier `prebuild-install` warning path from installs. A single upstream ONNX-runtime deprecation warning may still appear during npm install.
