...
AST 感知分块默认支持 `TypeScript`、`JavaScript`、`Python`、`Go`、`Bash`、`Lua` 和 `Dart`。其他语言也会通过基于行的 fallback 分块正常建立索引。
...
**共 52 个工具。** 所有工具都支持 `response_format: "json"` (默认) 或 `"markdown"`。列表类工具会返回 `total_count`、`has_more`、`next_offset` 用于分页。
...
| **代码检索** | 52 MCP 工具,AST 感知,混合搜索 | 无 | 无 | 无 | 无 |
...
| **运行时** | Node.js + TypeScript (轻量) | Python + ChromaDB | Python + Neo4j | Python + Neo4j | Python (云端) |
...
`0.1.0` 是一个重大版本 -- 完全用 TypeScript 重写 -- 包含时序知识图谱、多跳遍历、Nest/Branch 层级、代理级内存、语义去重、对话摄取、Hook 系统、CLI 优先架构（含 52 个 MCP 工具）、10 个 Claude Code 斜杠命令、交互式 TUI 仪表盘、引导式入门向导和端到端自测。Schema 迁移全部是增量且向后兼容的 -- 现有数据库在首次运行时自动升级。
