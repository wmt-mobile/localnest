...
AST 기반 청킹은 `TypeScript`, `JavaScript`, `Python`, `Go`, `Bash`, `Lua`, `Dart`에 기본으로 제공됩니다. 다른 언어도 줄 단위 fallback 청킹으로 정상적으로 인덱싱됩니다.
...
**총 52개 도구.** 모든 도구는 `response_format: "json"`(기본값) 또는 `"markdown"`을 지원합니다. 목록 계열 도구는 페이지네이션을 위해 `total_count`, `has_more`, `next_offset`을 반환합니다.
...
| **코드 검색** | 52 MCP 도구, AST 인식, 하이브리드 검색 | 없음 | 없음 | 없음 | 없음 |
...
| **런타임** | Node.js + TypeScript (경량) | Python + ChromaDB | Python + Neo4j | Python + Neo4j | Python (클라우드) |
...
`0.1.0`은 주요 릴리스입니다 -- TypeScript로 완전히 재작성되었습니다 -- 시계열 지식 그래프, 멀티홉 탐색, Nest/Branch 계층, 에이전트별 메모리, 시맨틱 중복 제거, 대화 수집, Hook 시스템, 52 MCP 도구가 포함된 CLI-first 아키텍처, 10개의 Claude Code 슬래시 명령, 대화형 TUI 대시보드, 가이드 온보딩 마법사, 엔드투엔드 셀프테스트가 포함됩니다. 스키마 마이그레이션은 모두 추가적이고 하위 호환이 가능합니다 -- 기존 데이터베이스는 첫 실행 시 자동으로 업그레이드됩니다.
