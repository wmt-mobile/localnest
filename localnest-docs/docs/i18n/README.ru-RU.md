...
AST-aware chunking включён по умолчанию для `TypeScript`, `JavaScript`, `Python`, `Go`, `Bash`, `Lua` и `Dart`. Другие языки тоже индексируются корректно благодаря построчному fallback chunking.
...
**Всего 52 инструмента.** Все tools поддерживают `response_format: "json"` (по умолчанию) или `"markdown"`. Tools со списками возвращают `total_count`, `has_more`, `next_offset` для пагинации.
...
| **Поиск по коду** | 52 MCP-инструмента, AST-aware, гибридный поиск | Нет | Нет | Нет | Нет |
...
| **Runtime** | Node.js + TypeScript (лёгкий) | Python + ChromaDB | Python + Neo4j | Python + Neo4j | Python (облако) |
| **Зависимости** | 0 новых (чистый SQLite) | ChromaDB (тяжёлый) | Neo4j ($25+/мес) | Neo4j | Cloud API |
| **MCP-инструменты** | 52 | 19 | 0 | 0 | 0 |
...
`0.1.0` -- крупный релиз, полностью переписанный на TypeScript, с временным графом знаний, многошаговым обходом, иерархией Nest/Branch, памятью с изоляцией по агенту, семантической дедупликацией, захватом разговоров, системой hooks, CLI-first архитектурой с 52 MCP-инструментами, 10 slash-командами Claude Code, интерактивным TUI-дашбордом, управляемым мастером onboarding и сквозным selftest. Миграции схемы аддитивны и обратно совместимы -- существующие базы обновляются автоматически при первом запуске.
