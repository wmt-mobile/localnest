...
AST-aware chunking ডিফল্টভাবে `TypeScript`, `JavaScript`, `Python`, `Go`, `Bash`, `Lua` এবং `Dart`-এর জন্য আসে। অন্য ভাষাগুলোও line-based fallback chunking দিয়ে পরিষ্কারভাবে index হয়।
...
**মোট 52টি tool।** সব টুল `response_format: "json"` (ডিফল্ট) বা `"markdown"` সমর্থন করে। list tool-গুলো pagination-এর জন্য `total_count`, `has_more`, `next_offset` ফেরত দেয়।
...
| **Code retrieval** | 52 MCP tool, AST-aware, hybrid search | নেই | নেই | নেই | নেই |
...
`0.1.0` একটি বড় release -- সম্পূর্ণভাবে TypeScript-এ পুনর্লিখিত -- temporal knowledge graph, multi-hop traversal, Nest/Branch hierarchy, agent-scoped memory, semantic dedup, conversation ingestion, hooks system, 52 MCP tool সহ CLI-first architecture, 10 Claude Code slash command, interactive TUI dashboard, guided onboarding wizard, এবং end-to-end selftest সহ। Schema migration সব additive এবং backward-compatible -- বিদ্যমান database প্রথম run-এ স্বয়ংক্রিয়ভাবে upgrade হয়।
