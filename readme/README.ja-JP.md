...
**合計 50 ツール。** すべてのツールは `response_format: "json"` (既定) または `"markdown"` をサポートします。一覧系ツールは、ページネーション用に `total_count`、`has_more`、`next_offset` を返します。
...
| **コード検索** | 50 MCP ツール、AST 対応、ハイブリッド検索 | なし | なし | なし | なし |
...
| **ランタイム** | Node.js (軽量) | Python + ChromaDB | Python + Neo4j | Python + Neo4j | Python (クラウド) |
