...
LocalNest, yapay zeka ajanlarına kodunuza güvenli ve sınırlı erişim veren, hibrit arama, anlamsal indeksleme, zamansal bilgi grafiği ve makinenizden hiç çıkmayan kalıcı bellek sağlayan local-first bir MCP sunucusu ve CLI aracıdır.

**52 MCP tool** | **Zamansal bilgi grafiği** | **Çok atlamalı grafik geçişi** | **Ajan kapsamlı bellek** | **Sıfır bulut bağımlılığı**
...
## 0.1.0'daki Yenilikler

> Tam changelog: [CHANGELOG.md](../CHANGELOG.md)

- **Zamansal bilgi grafiği** -- varlıklar, üçlüler, as_of sorguları, zaman çizelgeleri, çelişki algılama
- **Çok atlamalı grafik geçişi** -- özyinelemeli CTE'ler ile ilişkileri 2-5 atlama derinliğinde keşfedin (LocalNest'e özgü)
- **Nest/Branch hiyerarşisi** -- düzenli erişim için LocalNest'in kendi iki seviyeli bellek taksonomisi
- **Ajan kapsamlı bellek** -- özel günlük girişleri ile ajan bazında izolasyon
- **Anlamsal tekilleştirme** -- embedding benzerlik kapısı neredeyse aynı bellek kirliliğini önler
- **Konuşma alımı** -- varlık çıkarma ile Markdown/JSON sohbet dışa aktarımlarını içe aktarın
- **Hook sistemi** -- bellek, KG, geçiş, alım için ön/son işlem geri çağırmaları
- **CLI-first mimarisi** -- her şey için birleşik `localnest <noun> <verb>` komutları
- **Kabuk tamamlamaları** -- bash, zsh, fish sekme tamamlama
- **17 yeni MCP tool** (toplam 52) -- KG, yuvalar, geçiş, günlük, alım, tekilleştirme, hook'lar
...
| **Zamansal bilgi grafiği** | Zaman geçerliliği olan özne-yüklem-nesne üçlüleri -- herhangi bir anda neyin doğru olduğunu sorgulayın |
| **Çok atlamalı grafik geçişi** | Özyinelemeli CTE'ler ile ilişkileri 2-5 atlama derinliğinde keşfedin -- başka hiçbir yerel araç bunu yapamaz |
| **Nest/Branch hiyerarşisi** | Düzenli erişim için meta veri filtrelemeli boost'lu iki seviyeli bellek taksonomisi |
| **Konuşma alımı** | Markdown/JSON sohbet dışa aktarımlarını yapılandırılmış bellek + KG üçlülerine aktarın |
| **Ajan izolasyonu** | Ajan başına günlük ve bellek kapsamı -- birden fazla ajan, sıfır çapraz kontaminasyon |
| **Hook sistemi** | Bellek, KG, geçiş, alım için ön/son işlem hook'ları -- kendi mantığınızı bağlayın |
...
AST-aware chunking varsayılan olarak `TypeScript`, `JavaScript`, `Python`, `Go`, `Bash`, `Lua` ve `Dart` için gelir. Diğer diller de satır tabanlı fallback chunking ile temiz biçimde indekslenir.
...
Dört iş akışı neredeyse her şeyi kapsar:
...
### Bilgi grafiği -- proje hakkında yapılandırılmış gerçekler

```
localnest_kg_add_triple   → bir gerçeği kaydet: "auth-service" "JWT" kullanır
localnest_kg_query        → "auth-service" neyle ilişkili?
localnest_kg_as_of        → 1 Mart'ta bu konuda ne doğruydu?
localnest_graph_traverse  → bağlantıları keşfetmek için 2-3 hop dolaş
```

### Konuşma belleği -- önceki sohbetlerden öğrenin

```
localnest_ingest_markdown → bir konuşma dışa aktarmasını içe aktar
localnest_memory_recall   → bununla ilgili ne biliyorum?
localnest_diary_write     → bu ajan için özel çizim tahtası
```
...
**Global flag'lar** her komutta çalışır: `--json` (makine çıktısı), `--verbose`, `--quiet`, `--config <path>`
...
| Tool | Ne yapar |
|------|-------------|
| `localnest_kg_add_entity` | Varlıklar oluşturur (kişiler, projeler, kavramlar, araçlar) |
| `localnest_kg_add_triple` | Zamansal geçerlilik ile özne-yüklem-nesne gerçekleri ekler |
| `localnest_kg_query` | Yön filtreleme ile varlık ilişkilerini sorgular |
| `localnest_kg_invalidate` | Bir gerçeği artık geçerli değil olarak işaretler (arşivleme, silme değil) |
| `localnest_kg_as_of` | Zaman noktası sorguları -- X tarihinde ne doğruydu? |
| `localnest_kg_timeline` | Bir varlık için kronolojik gerçek evrimi |
| `localnest_kg_stats` | Varlık sayısı, üçlü sayısı, yüklem dağılımı |

### Nest/Branch Organizasyonu

| Tool | Ne yapar |
|------|-------------|
| `localnest_nest_list` | Sayılarla birlikte tüm yuvaları (üst düzey bellek alanları) listeler |
| `localnest_nest_branches` | Bir yuva içindeki dalları (konuları) listeler |
| `localnest_nest_tree` | Tam hiyerarşi: yuvalar, dallar ve sayılar |

### Grafik Geçişi

| Tool | Ne yapar |
|------|-------------|
| `localnest_graph_traverse` | Yol izleme ile çok atlamalı geçiş (özyinelemeli CTE) |
| `localnest_graph_bridges` | Çapraz yuva köprülerini bulur -- alanlar arası bağlantılar |

### Ajan Günlüğü

| Tool | Ne yapar |
|------|-------------|
| `localnest_diary_write` | Özel not defteri girişi yazar (ajan izolasyonlu) |
| `localnest_diary_read` | Kendi son günlük girişlerinizi okur |

### Konuşma Alımı

| Tool | Ne yapar |
|------|-------------|
| `localnest_ingest_markdown` | Markdown konuşma dışa aktarımlarını bellek + KG'ye aktarır |
| `localnest_ingest_json` | JSON konuşma dışa aktarımlarını bellek + KG'ye aktarır |
| `localnest_memory_check_duplicate` | Kayıt öncesi anlamsal tekrar algılama |
...
**Toplam 52 tool.** Tüm tools `response_format: "json"` (varsayılan) veya `"markdown"` destekler. Liste tools'ları sayfalama için `total_count`, `has_more`, `next_offset` döndürür.

---

## LocalNest Nasıl Karşılaştırılır

LocalNest, kod erişimi VE yapılandırılmış belleği tek bir araçta birleştiren tek local-first MCP sunucusudur. Konumu şöyledir:

| Yetenek | LocalNest | MemPalace | Zep | Graphiti | Mem0 |
|---|---|---|---|---|---|
| **Local-first (bulut yok)** | Evet | Evet | Hayır ($25+/ay) | Hayır (Neo4j) | Hayır ($20-200/ay) |
| **Kod erişimi** | 52 MCP tool, AST-aware, hibrit arama | Yok | Yok | Yok | Yok |
| **Bilgi grafiği** | Zamansal geçerlilikli SQLite üçlüleri | SQLite üçlüleri | Neo4j | Neo4j | Key-value |
| **Çok atlamalı geçiş** | Evet (özyinelemeli CTE, 2-5 atlama) | Hayır (sadece düz arama) | Hayır | Evet (Neo4j gerekir) | Hayır |
| **Zamansal sorgular (as_of)** | Evet | Evet | Evet | Evet | Hayır |
| **Çelişki algılama** | Evet (yazma zamanı uyarıları) | Var ama bağlı değil | Hayır | Hayır | Hayır |
| **Konuşma alımı** | Markdown + JSON | Markdown + JSON + Slack | Hayır | Hayır | Hayır |
| **Ajan izolasyonu** | Ajan başına kapsam + özel günlük | Wing-per-agent | User/session scoping | Hayır | User/agent/run/session |
| **Anlamsal tekilleştirme** | Tüm yazmalarda 0.92 cosine kapısı | 0.9 eşik | Hayır | Hayır | Hayır |
| **Bellek hiyerarşisi** | Nest/Branch (orijinal) | Wing/Room/Hall (palace) | Düz | Düz | Düz |
| **Hook sistemi** | Ön/son işlem hook'ları | Yok | Webhooks | Yok | Yok |
| **Runtime** | Node.js + TypeScript (hafif) | Python + ChromaDB | Python + Neo4j | Python + Neo4j | Python (bulut) |
| **Bağımlılıklar** | 0 yeni (saf SQLite) | ChromaDB (ağır) | Neo4j ($25+/ay) | Neo4j | Cloud API |
| **MCP tools** | 52 | 19 | 0 | 0 | 0 |
| **Maliyet** | Ücretsiz | Ücretsiz | $25+/ay | $25+/ay | $20-200/ay |

**LocalNest'in benzersiz konumu:** Yapay zekanıza hem derin kod anlayışı hem de yapılandırılmış kalıcı bellek veren tek araç -- tamamen yerel, sıfır bulut, sıfır maliyet.
...
**Bilgi grafiği:** Yapılandırılmış gerçekleri zamansal geçerlilik ile özne-yüklem-nesne üçlüleri olarak saklayın. `as_of` ile herhangi bir zaman noktasında neyin doğru olduğunu sorgulayın. Özyinelemeli CTE geçişi ile ilişkileri 2-5 atlama derinliğinde keşfedin. Yazma zamanında çelişkileri algılayın.

**Nest/Branch hiyerarşisi:** Bellekleri yuvalara (üst düzey alanlar) ve dallara (konular) düzenleyin. Meta veri filtrelemeli geri çağırma, puanlama öncesinde adayları daraltarak daha hızlı ve daha doğru sonuçlar sağlar.

**Ajan izolasyonu:** Her ajan kendi bellek kapsamına ve özel günlüğüne sahip olur. Geri çağırma kendi + global bellekleri döndürür, asla başka bir ajanın özel verilerini döndürmez.

**Anlamsal tekilleştirme:** Her yazma, embedding benzerlik kapısından (varsayılan 0.92 cosine eşik) geçer. Neredeyse aynı girişler depolamadan önce yakalanır -- belleğiniz temiz kalır.

**Konuşma alımı:** Markdown veya JSON sohbet dışa aktarımlarını içe aktarın. Her tur, otomatik varlık çıkarma ve KG üçlü oluşturma ile bir bellek girişi olur. Aynı dosyanın yeniden alımı içerik hash'i ile atlanır.

**Hook'lar:** Herhangi bir bellek işlemine ön/son geri çağırma kaydedin -- depolama, geri çağırma, KG yazmaları, geçiş, alım. Çekirdek kodu değiştirmeden özel boru hatları oluşturun.
...
`0.1.0` buyuk bir surumdur -- tamamen TypeScript ile yeniden yazilmistir -- temporal knowledge graph, multi-hop traversal, Nest/Branch hiyerarsisi, ajan kapsamli bellek, semantik tekillistirme, konusma alimi, hook sistemi, 52 MCP tool ile CLI-first mimarisi, 10 Claude Code slash komutu, interaktif TUI panosu, rehberli onboarding sihirbazi ve uctan uca selftest icerir. Sema gecisleri tamamen eklemeli ve geriye uyumludur -- mevcut veritabanlari ilk calistirmada otomatik olarak yukseltilir.
