...
test('release-test-installed-runtime default config parameterizes report paths by version label', async () => {
  const mod = await import('../scripts/release/release-test-installed-runtime.mjs');
  const config = mod.__test_buildDefaultConfig({ versionLabel: '0.0.4' });

  assert.match(config.markdownReportPath, /0-0-4-release-test-report\.md$/);
  assert.match(config.jsonReportPath, /0-0-4-release-test-report\.json$/);
  assert.equal(config.versionLabel, '0.0.4');
});
